...
DQN → PPO → SAC, each a standalone milestone with a headline metric against a published baseline. Vanilla DQN is discrete-only and SAC is continuous-only, so the suite runs on two tracks with PPO as the bridge:
...
A Connect 4 self-play on-ramp closes the suite: the self-play loop, checkpoint pool and Elo harness, built where a run takes minutes and an exact solver gives ground truth.
...
- **Minimal dependencies**, pinned. CPU by default.
...
| 4 | Connect 4 self-play: opponent pool, checkpoint Elo harness | **done** — naive self-play forgets (no proxy overlap vs the pool arm), measured on an exact-oracle instrument stack (see Results) |
...
- **Sample efficiency is not free, and the figure's second row is the price.**
  ...
  whether samples or compute is the scarce resource — and for any environment
  where a single sample costs a network round-trip, that is the ballgame.
...
## Results — Phase 4: self-play on Connect 4
...
Self-play is a genuinely new *mechanism*, so it gets its own phase on an
environment where a 2M-step run takes ~5 minutes and ground truth exists. The
learner trains only against frozen snapshots of itself held in an opponent
pool (20 deep, strided retention so the step-0 snapshot anchors the span,
80/20 latest/historical draw); the **naive arm is a one-key config diff** —
`pool_size: 1`, a lagged copy of the learner, which is what the self-play
literature means by naive. Deliberately **PPO-only, no MCTS**: tree search
needs a forward model, which many interesting environments do not provide.
...
- **The ceiling is distribution, not architecture.** No training variant —
  ...
  broadening where the policy lives.
...
- **The forgetting result does not transfer by default.** Connect 4 is
  deterministic, perfect-information, and alternating-move; stochastic,
  imperfect-information, simultaneous-move games are the exact class OpenAI
  Five scopes strategy collapse to. What transfers is the machinery and the
  instruments, which is what they were built for.
